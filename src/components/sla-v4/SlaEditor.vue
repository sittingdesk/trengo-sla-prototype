...
const { nav, goOverview } = useSlaNavV4()
const { slas, byId, upsert } = useSlaPoliciesV4()
...
/** Tooltip for the target row — the only per-type copy not living in the meta. */
const targetTooltip =
  slaType === 'first_response' ? SLA_TOOLTIPS.firstReply : SLA_TOOLTIPS.resolution
...
const availableOptions = computed(() => {
  const field = CUSTOM_FIELDS.find((f) => f.id === target.value.fieldId)
  if (!field) return []
  const used = new Set(target.value.rows.map((r) => r.optionId))
...
watch(
  ...
  (fieldId) => {
    ...
    const d = target.value.default
    target.value.rows = field
      ? field.options.map((o) => ({
...
function removeRow(id: string) {
  target.value.rows = target.value.rows.filter((r) => r.id !== id)
}
        ...
        >
          <template #below>
            <div class="mt-4 pl-11">
              ...
              <div
                v-for="row in (hasCustomFields ? draft.target.rows : [])"
                :key="row.id"
                class="flex items-center gap-3 border-b border-grey-200 py-2"
              >
                <span class="min-w-0 flex-1 truncate text-sm font-medium text-grey-700">
                  {{ optionLabel(draft.target.fieldId, row.optionId) }}
                </span>
                <div class="flex items-center gap-2">
                  <Input
                  ...
                  />
                  <UnitSelect v-model="row.unit" class="w-[119px]" :options="typeMeta.units" />
                </div>
                <button
                  type="button"
                  :aria-label="`Remove ${optionLabel(draft.target.fieldId, row.optionId)}`"
              ...
              <Popover v-if="hasCustomFields && draft.target.fieldId" v-model:open="addMenuOpen">
