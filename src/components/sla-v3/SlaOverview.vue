...
// SlaOverview (Iteration 3) — ONE list of all SLAs (first response + resolution
...
import { channelsLabel, coverage, sortSlas } from '@/lib/slaV3'
import { SLA_TYPES, SLA_TYPE_META, type Sla, type SlaType } from '@/data/slaDataV3'
import PolicyRow from './PolicyRow.vue'
import PolicyRowSkeleton from './PolicyRowSkeleton.vue'

const { goEditor, goCreate } = useSlaNavV3()
const { slas, toggleActive, removeSla } = useSlaPoliciesV3()
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <template v-if="pendingDelete?.active && pendingDelete.channels.length">
              Conversations on {{ channelsLabel(pendingDelete.channels) }} will no longer have a
