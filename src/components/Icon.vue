 ...
 *
 * Sources: svg icons/linear/*  (outline, stroke=currentColor) — default
 *          svg icons/filled/*  (solid, fill=currentColor) — variant="filled"
 *
 * Names are fuzzy-matched: case-insensitive, ignoring separators and the
 * decorations 'icon' / 'outline' / 'filled'. Colour comes from the surrounding
 * text colour (currentColor) — set it with a text-* utility. Render sizes per
 * design.md §8: 16 / 20 / 32.
 */
...
function buildMap(glob: Record<string, string>): Record<string, string> {
  ...
  for (const [path, svg] of Object.entries(glob)) {
    const key = normalize(path.split('/').pop() ?? '')
    if (key) map[key] = svg
  }
  return map
}

const maps = { linear: buildMap(linear), filled: buildMap(filled) }

const svg = computed(() => {
  const found = maps[props.variant][normalize(props.name)]
